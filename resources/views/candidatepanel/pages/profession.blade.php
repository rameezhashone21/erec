...
@section('content')
    ...
    <div class="col-xl-9 col-lg-8">
        ...
        <div class="dashboard_content bg-white rounded_10 p-md-4 p-2">
            <div class="d-md-flex aling-items-center mb-3">
                        ...
                        <div class="modal-content">
                            <div class="modal-body">
                                ...
                                <form id="firstform-create" class="dashboard-form" method="post"
                                    action="{{ route('candidates.profession.update') }}" enctype="multipart/form-data"
                                    name="myForm">
                                    @csrf
                                    @method('post')
                ...
                {{-- <a class="btn_viewall fw-500 px-4 py-2 d-inline-block ms-auto"
                    href="{{ route('candidates.profession.new') }}"> Add New Experience
      </a> --}}
            </div>
            @if (isset(auth()->user()->candidatePro))
                @foreach ($user->candidatePro as $key => $row)
                    <div class="border-bottom py-3" id="proDiv-{{ $row->id }}">
                        <div class="d-md-flex">
                            ...
                            <div class="flex-grow-1">
                                <div class="d-flex justify-content-between">
                                    <h3 class="fw-500 fs-5 mb-1" id="designationText-{{ $row->id }}">
                                        {{ $row->designation }}
                                    </h3>
                                    <a class="me-3 d-inline-block ms-auto text_grey_999" data-bs-toggle="tooltip"
                                        data-bs-placement="top" onclick="btn_edit({{ $row->id }})" title="Edit"
                                        href="javascript:;">
                                    ...
                                    {{-- <a class="d-inline-block ms-auto text_grey_999" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title="Edit"
                                        href="{{ route('candidates.professionupdate', $row->id) }}"> <i class="fa-solid fa-pencil "></i>
            </a> --}}
                                    {{-- start edit experience modal --}}

                                    {{-- end edit experience modal --}}
                                </div>
                                <h4 class="fw-500" id="company_nameText-{{ $row->id }}">{{ $row->company_name }}</h4>
              ...
              date_create($row->year_exp))->format('%y
              years, %m months') < 2) {{ date_diff(date_create($row->month_exp), date_create($row->year_exp))->format('%y year, %m months') }} @else {{ date_diff(date_create($row->month_exp), date_create($row->year_exp))->format('%y years, %m months') }} @endif @else {{ \Carbon\Carbon::parse($row->month_exp)->isoFormat('MMM YY') }} - Currently working here. @endif --}} @if ($row->year_exp != 0)
                                            @if (date_diff(date_create($row->month_exp), date_create($row->year_exp))->format('%y
                                                                                                                                                                                                                                                                                                                                                                                                                                                  years, %m months') <
                                                    1)
                                                {{-- {{ date_diff(date_create($row->month_exp), date_create($row->year_exp))->format('%m months') }} --}} <p>Less than a month</p>
                                ...
                                <div class="text_grey_999 fs-14 mt-1 d-flex align-items-center" style="gap: 0 8px;">
                                    ...
                                    <span id="comp_locText-{{ $row->id }}">
                                        Located:
                                        {{ $row->comp_loc }}
                                    </span>
                                </div>
                                <p class="text_grey_999 fs-14 mt-3" id="descriptionText-{{ $row->id }}">
                                    {!! $row->description !!}
                                </p>
                            </div>
                        </div>
    ...
    {{-- <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body">
                    <p class="text-center fw-bold fs-4">Are you sure you want to delete?</p>
                </div>
                <div class="modal-footer border-0 justify-content-center">
                    <a id="delete-edu" href="" class="btn btn-danger">Yes</a>
...
@section('bottom_script')
    ...
    <script>
        function btn_edit(value) {
            // console.log(value);
            var url = '{{ route('candidates.professionupdate', ': id ') }}';
        ...
        $(document).ready(function() {
            ...
            $('.dashboard-form').on('submit', function(e) {
                ...
                if (!$("#currentlyWorkHere").is(":checked") && $("#toDate").val() == '') {
                    ...
                    $.ajax({
                            url: "{{ route('candidates.profession.update') }}",
                            type: "POST",
                            data: userFormData,
                            dataType: "json",
                            encode: true,
                            ...
                            if (data[0] != "error") {
                                if (formid != "firstform-create") {
                                    ...
                                    html += "{{-- start edit experience modal --}}";
                                    html += "";
                                    html += "{{-- end edit experience modal --}}";
                                    html += "</div>";
                                    ...
                                    html +=
                                        "<div class='text_grey_999 fs-14 mt-3 d-flex align-items-center' style='gap: 0 8px;'>";
                                    html += "{{-- <i class='fa-solid fa-calendar fs-5 me-2'></i> --}}";
                                    html +=
                                    ...
                                    html +=
                                        "<div class='text_grey_999 fs-14 mt-1 d-flex align-items-center' style='gap: 0 8px;'>";
                                    html += "{{-- <i class='fa-solid fa-building-columns fs-5 me-2'></i> --}}";
                                    html +=
