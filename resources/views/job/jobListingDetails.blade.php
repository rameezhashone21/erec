    ...
    <section>
      <div class="container pt-4">
        <div class="row justify-content-center py-4 mt-5">
          {{-- <div class="col-lg-12">
                        <h1 class="mb-0 fs-48 text-center fw-bold text-uppercase py-5"> Job Detail</h1>
                    </div> --}}
          <div class="col-lg-8">
                  ...
                  @if (auth()->user()->role != 'admin')
                    <div>
                      @if (auth()->user()->role === 'company')
                        <span>Back to </span> <a href="{{ route('company.dashboard') }}" class="text-primary">Dashboard</a>
                      @elseif(auth()->user()->role === 'rec')
                        <span>Back to </span> <a href="{{ route('dashboard') }}" class="text-primary">Dashboard</a>
                      @elseif(auth()->user()->role === 'user')
                        <span>Back to </span> <a href="{{ route('candidate.dashboard') }}"
                          class="text-primary">Dashboard</a>
                      @endif
                    </div>
                  @endif
                @endif
                <span class="title text-theme-primary fw-bold">{{ $app->post }}
                  {{-- <img src="{{ asset('imgs/tick.svg') }}" alt="tick" class="ps-2" width="30"> --}}
                </span>
            ...
            <img src="{{ asset('storage/' . $app->banner) }}" alt="" class='w-100 d-block mb-4'
              style='height: 200px; object-fit: cover;'>
            <div class="mb-4">
              <h4 class="fs-18 fw-600 mb-3">Job Description</h4>
              <p class="fs-14 text-secondary mb-0">{!! $app->description !!}</p>
            </div>
            <div class="mb-4">
            ...
            <div class="social-btn-sp social__icons__detail d-flex align-items-center">
              ...
              {!! Share::page(route('job.listing.details', $app->slug))->facebook()->twitter()->whatsapp()->linkedin() !!}
            </div>
          </div>
          <div class="col-lg-4 mt-3 mt-lg-0">
            <div class="d-flex justify-content-md-between text-center align-items-center pb-4">
                                ...
                                @if (auth()->user()->role == 'user')
                                    ...
                                    @else
                                        <button disabled style="opacity: 0.5;"
                                            class="btn open-apply-modal default-btn w-100 mb-3" id="{{ $app->id }}"
                                            href="javascript:;">Applied</button>
                                    @endif
                                @endif
                            @elseif (auth()->user()->role == 'company' && auth()->user()->role == 'rec')
                                <a class="btn default-btn w-100 mb-3" href="{{ route('job.session') }}">Apply Now</a>
                            @endif --}}
              @if (auth()->check())
                {{-- {{ dd('ok') }} --}}
                @if (auth()->user()->candidate != null)
                  @if (auth()->user()->role == 'user')
                    @if ($app->jobApp == '[]')
                      <a class="btn open-apply-modal default-btn default-btn-swip w-100 mb-3" id="{{ $app->id }}"
                        href="javascript:;">
                        Apply
                        Now</a>
                    @else
              ...
              @else
                <a class="btn default-btn w-100 mb-3" href="{{ route('job.session') }}">Apply
                  Now</a>
              @endif
              @if (Auth::check())
                @if (isset(auth()->user()->candidate))
                  @if (isset(auth()->user()->wishlist($app->id)->post_id))
                    <div class="singlePost">
                      <a href="" onclick="wishlist_singlePost({{ $app->id }})"
                ...
                @endif
              @else
                <a href="{{ route('login') }}" class="btn default-btn-2 mb-3">
            ...
            <div class="search-area mb-3 p-4 mb-3 shadow">
              ...
              <ul>
                <li class="d-flex align-items-center pb-4">
                  <img src="{{ asset('imgs/date.svg') }}" alt="date" style='width: 30px; height: 20px'>
                  <div class="ps-4">
                    <p class="mb-0 fw-600 fs-18"> Date Posted </p>
                    <p class=" mb-0 text-grey fs-14 ">{!! htmlspecialchars_decode(date('j<\s\up>S</\s\up> F Y', strtotime($app->created_at))) !!}</p>
                  </div>
                </li>
                <li class="d-flex align-items-center pb-4">
                  <img src="{{ asset('imgs/location.svg') }}" alt="location" style='width: 30px; height: 20px'>
                  <div class="ps-4">
                ...
                <li class="d-flex align-items-center pb-4">
                  <img src="{{ asset('imgs/money.svg') }}" alt="money" style='width: 30px; height: 20px'>
                  <div class="ps-4">
                ...
                {{-- <li class="d-flex align-items-center pb-4">
                                    <img src="{{ asset('imgs/expire-date.svg') }}" alt="expire-date" style='width: 30px; height: 20px'>
                                    <div class="ps-4">
                ...
                <li class="d-flex align-items-center pb-4">
                  <img src="{{ asset('imgs/experience.svg') }}" alt="experience" style='width: 30px; height: 20px'>
                  <div class="ps-4">
                ...
                <li class="d-flex align-items-center pb-4">
                  <img src="{{ asset('imgs/gender.svg') }}" alt="gender" style='width: 30px; height: 20px'>
                  <div class="ps-4">
                    <p class="mb-0 fw-600 fs-18"> Gender </p>
                    <p class=" mb-0 text-grey fs-14 text-capitalize">{{ $app->gender }}</p>
                  </div>
                </li>
                <li class="d-flex align-items-center pb-4">
                  <img src="{{ asset('imgs/qualification.svg') }}" alt="qualification"
                ...
                <ul>
                  @if ($app->company != null)
                    <li class="d-lg-flex align-items-center">
                      <div>
                        <img src="{{ asset('storage/' . $app->company->logo) }}" alt="bag"
                          class="profile__image__detail">
                      </div>
                      <div class="ps-3">
                        <p class="mb-0 fw-bold">{{ $app->company->name }}</p>
                        <p class="mb-0">{{ $app->company->type }}</p>
                        <a href="{{ route('job.details', $app->company->slug) }}"
        ...
        <div class="row">
          <div class="col-lg-10">
                    ...
                    <div class="btns d-flex flex-column">
                                                ...
                                                @if (auth()->user()->role == 'user')
                                                    @if ($row->jobApp == '[]')
                                                        <a class="btn open-apply-modal default-btn mb-3"
                                                            ...
                                                            Now</a>
                                                    @else
                                                        <button disabled style="opacity: 0.5; pointer-event: none;"
                                                            class="btn open-apply-modal default-btn mb-3"
                                                            id="{{ $row->id }}" href="javascript:;">Applied</button>
                                                    @endif
                                                @endif
                                            @elseif (auth()->user()->role == 'company' && auth()->user()->role == 'rec')
                      ...
                      @if (auth()->check())
                        ...
                        @if (auth()->user()->candidate != null)
                          @if (auth()->user()->role == 'user')
                            @if ($row->jobApp == '[]')
                              <a class="btn open-apply-modal default-btn mb-3" id="{{ $row->id }}"
                      ...
                      {{-- <a class="btn default-btn-2" href="{{ asset('public/candidateDoc/doc/'.$row->resume->document) }}" target="_blank">View Resume</a> --}}
                      <div class="d-flex justify-content-center align-items-center mt-3" style="gap: 0 20px;">
                        ...
                        @if ($row->company != null)
                          <a href="mailto:{{ $row->company->user->email }}" class="icons__box"><i
                        ...
                        @if (Auth::check())
                          @if (isset(auth()->user()->candidate))
                            ...
                            @else
                              <div class="wishlist_box-{{ $row->id }}">
                                <a href="" class="icons__box" onclick="wishlist_post({{ $row->id }})">
                                  <i class="fas fa-heart"></i>
                                </a>
                              </div>
                            @endif
                          @endif
                        @else
                          <a href="{{ route('login') }}" class="icons__box"><i class="fas fa-heart"></i>
                          </a>
                        @endif
                      </div>
                    </div>
            ...
            <p class="text-center mb-0 py-4">
              <a href="{{ route('job.browse') }}" class="btn default-btn ">Other Posted Jobs</a>
            </p>
          </div>
        </div>
      </div>
