                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-3 col-md-4">
                            ...
                            <div class="row row-cols-1 form__search__box search-area mb-3 py-4 px-3 d-block" id="search_comp">
                                ...
                                <div class="position-relative">
                                    <input type="text" name="search_comp"
                                        class="w-100 fs-14 bg-theme-secondary  text-dark h-50px mb-3"
                                        placeholder="Search Employer" value="{{ $search_word }}" />
                                ...
                                <div class="col ">
                                    <div class="position-relative">
                                        <input type="text" name="search_comp" id="search-title" autocomplete="off"
                                            class="w-100 fs-14 bg-theme-secondary text-dark h-50px mb-3"
                                            {{-- onchange="form.submit()" --}} placeholder="Enter employer name here"
                                            value="{{ $search_word }}">
                                        <img src="{{ asset('images/icon-search.svg') }}" alt=""
                                            class="img-fluid position-absolute">
                                ...
                                <div class="search-area mb-3 py-3 px-3 mb-3">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <a data-bs-toggle="collapse" href="#Designation" role="button"
                                            ...
                                            aria-controls="Designation" id="collapseButtonOne" {{-- class="@if (count($value) <= 0) collapsed @endif"> --}}
                                            class="">
                                            <i class="fas fa-plus" aria-hidden="true"></i>
                                        </a>
                                    </div>
                                    {{-- <div class="collapse @if (count($value) > 0) show @endif" id="Designation"> --}}
                                                    ...
                                                    <li class="d-flex justify-content-between align-items-center py-2">
                                                        <div class="form-check">
                                                            <input class="form-check-input rounded-pill" type="checkbox"
                                                                value="{{ $row->id }}" name="search_skills[]"
                                                                id="fjkl{{ $row->id }}"
                                                                @foreach ($value as $col)
                                                            @if ($row->id == $col)
                                                                checked=checked
                                                            @endif @endforeach />
                                                            <label class="form-check-label" for="fjkl{{ $row->id }}">
                                                                {{ $row->name }}
                                                            </label>
                                                        </div>
                                                        {{-- <span> 1 </span> --}}
                        ...
                        <div class="col-lg-9 col-md-8">
                            <div class="row align-items-center justify-content-between gy-3 mb-4">
                                <div class="col-lg-4">
                                    <h3 class="view_profile_description fs-16 mb-0">Showing
                                        {{-- Showing 1-{{ count($comp) }} Of {{ $comp->total() }} Employers --}}
                                        @if (count($comp) > 0)
                                            1-
                                        @endif
                                        {{ count($comp) }} Of {{ $comp->total() }} Employers
                                    </h3>
                                </div>
                                <div class="col-lg-7">
                                    <form action="{{ route('employer.list') }}" method="get">
                                            ...
                                            <div class="new-recruter-box">
                                                @if ($row->user->banner != null)
                                                    <img src="{{ asset('storage/' . $row->user->banner) }}"
                                                        alt="{{ $row->name }}" class="user_banner img-fluid">
                                                @else
                                                    <img src="{{ asset('dashboard/images/Company.png') }}"
                                                        alt="{{ $row->name }}" class="user_banner img-fluid">
                                                @endif
                                                <div class="text-center px-3 pb-4">
                                                    ...
                                                    @endif
                                                    <p class="job_type mb-3">{{ $row->type }}</p>
                            ...
                            <div class="d-flex justify-content-center pt-5">
                                {{ $comp->appends(request()->except(['page', '_token']))->links() }}
                            </div>
