              ...
              @foreach ($jobApp as $key => $row)
                @if (isset($row->post))
                  <tr>
                    ...
                    <td class="jopAssignSelectbox" id="td_id{{ $row->id }}">
                      @if ($row->status == 0)
                        ...
                        <select name="assign_test" onchange="assign_test({{ $row->id }})"
                          id="assign_test{{ $row->id }}" class="select2-multiple form-control fs-14  h-50px"
                          required>
                          <option selected disabled value="">Select Test</option>
                          @foreach ($row->test($row->class_id) as $col)
                            <option value="{{ $col['id'] }}">{{ $col['exam_title'] }}</option>
                    ...
                    <td>
                      @if ($row->status == 0)
                        <button role="button" id="blue_shortlist{{ $row->id }}" class="btn btn-success btn-sm">
                          <i class="fa-solid fa-exclamation"></i>
                          Task Not Assigned Yet.
                        </button>
                      @elseif($row->status == 1)
                        <button role="button" class="btn btn-success btn-sm">
                          <i class="fa fa-check" id="green_shortlist{{ $row->id }}" style="font-size:18px"></i>
                          Shortlist
                        </button>
                      @endif
                    </td>
